...
SYS_SUMMARY = (
    ...
    "Правила:\n"
    "- Не выдумывай фактов; если сведений мало — просто сделай более общий и осторожный пересказ только по имеющимся сведениям.\n"
...
def summarize_document(owner_id: int, doc_id: int) -> str:
    ...
    # Никогда не отдаём пустую строку наружу
    reply = (reply or "").strip()
    if not reply:
        return (
            "Не удалось получить резюме от модели. "
