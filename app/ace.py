...
SYS_ANSWER = (
    ...
    "Отвечай максимально полно и подробно, опираясь на этот документ. "
    "Если сведений недостаточно — дай частичный, но корректный ответ по имеющимся данным, ничего не придумывая. "
...
SYS_EDITOR = (
    ...
    "не придумывая недостающие детали и не перечисляя отдельно, каких данных не хватает. "
...
SYS_EXPLAIN = (
    ...
    "Если сведений мало — дай аккуратное общее объяснение только по тому, что есть в тексте, "
    "и при необходимости можешь порекомендовать, какие разделы обычно раскрывают подробнее.\n\n"
    "Формат:\n"
...
SYS_FULLREAD = (
    ...
    "Отвечай строго по этому тексту, без внешних фактов. Если данных недостаточно — дай частичный ответ по тому, что есть, "
    "не придумывая деталей. Если вопрос про таблицы/рисунки — опирайся на подписи и прилегающий текст; "
    "не придумывай номера/значения. Цитируй короткими фрагментами при необходимости, без указания страниц."
)
...
SYS_FULLREAD_REDUCE = (
    ...
    "Если данных не хватает — дай осторожный частичный ответ только по имеющимся фактам."
)
...
SYS_PART_ANSWER = (
    ...
    "Если сведений не хватает — дай частичный ответ по тому, что есть, ничего не придумывая. "
...
SYS_MERGE = (
    ...
    "(краткий вывод; 3–7 пунктов обоснования). "
