...
def generate_answer(
    ...
    *,
    ...
    if q:
        ...
        if many_parts or long_query or multi_questions:
            q_for_model += (
...
async def generate_answer_stream(
    ...
    *,
    ...
    # 🔧 Если запрос длинный / перечислительный — просим модель явно пройтись по всем пунктам.
    q_for_model = q
    if q:
        many_parts = (q.count(" и ") + q.count(",") + q.count(";")) > 3
        long_query = len(q) > 200
    ...
    system_prompt = (
        ...
        "того, чего в нём нет.\n"
        "Структурируй ответ под запрос: используй списки, подпункты и подзаголовки, когда это уместно.\n"
        "Если в вопросе перечислены несколько пунктов или аспектов, обязательно ответь по каждому из них "
        "отдельным подпунктом, без пропусков. Лучше дать немного больше пояснений, чем слишком мало.\n"
        "Если в вопросе упомянуты конкретные таблицы, рисунки или главы (например: «опиши рисунок 2.2, "
        "таблицу 2.1 и главу 1»), делай отдельные подпункты для каждого объекта в указанном порядке и "
        "поясняй их содержательно."
    )
