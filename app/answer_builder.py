...
_DEFAULT_RULES = (
    ...
    "6) Не придумывай факты вне блока Facts (Tables/Figures/TablesRaw/values); если данных нет — скажи честно: «В документе не найдено».\n"
    ...
    "9) Для диаграмм и графиков сначала коротко поясни, что показывают оси, легенда и единицы измерения, как устроен масштаб. "
    "Если в [Figures]/values нет явного описания масштаба, не придумывай его.\n"
    "10) Запрещено добавлять общие теоретические пояснения «от себя». Весь ответ должен опираться на [Facts].\n"
...
def facts_to_prompt(
    ...
    *,
    ...
    if bool((facts or {}).get("no_grounding")):
...
def generate_answer(
    ...
    *,
    ...
    # --- NEW: вытаскиваем явные подпункты из вопроса, чтобы потом проверить покрытие ---
    def _extract_expected_items(qtext: str) -> list[str]:
        """
        Пытаемся вытащить перечисление пунктов из вопроса:
        (1) ... (2) ...
        1) ... 2) ...
        1. ... 2. ...
        Возвращаем список "ключей" пунктов (для контроля), либо пусто.
        """
        qt = (qtext or "").strip()
        if not qt:
            return []
        ...
        # кейс: "(1) ... (2) ..."
        m = re.findall(r"\(\s*(\d{1,2})\s*\)\s*", qt)
        if m and len(m) >= 2:
            return [str(int(x)) for x in m]

        # кейс: "1) ... 2) ..."
        m = re.findall(r"(?m)^\s*(\d{1,2})\)\s+", qt)
        if m and len(m) >= 2:
            return [str(int(x)) for x in m]

        # кейс: "1. ... 2. ..." (но не годы типа 2024.)
        m = re.findall(r"(?m)^\s*(\d{1,2})\.\s+", qt)
        if m and len(m) >= 2:
            return [str(int(x)) for x in m]

        # кейс: "Дай отдельно: (1) ... (2) ..."
        m = re.findall(r"(?i)отдельно:\s*(?:\(|)(\d{1,2})(?:\)|)\s*", qt)
        if m and len(m) >= 2:
            return [str(int(x)) for x in m]

        # если просто "и" / запятые — НЕ считаем это явным списком, чтобы не ломать
        return []

    expected_items = _extract_expected_items(q)
    expected_n = len(expected_items)

    # 🔧 Для многоаспектных запросов просим модель пройтись по всем пунктам.
    q_for_model = q
    many_parts = (q.count(" и ") + q.count(",") + q.count(";")) > 3
    long_query = len(q) > 200
    multi_questions = q.count("?") > 1

    if expected_n >= 2:
        # NEW: если пункты явно пронумерованы — требуем тот же формат
        q_for_model += (
            "\n\nОтветь СТРОГО в формате пунктов 1), 2), 3)... "
            "Каждый пункт должен соответствовать одноимённому пункту из вопроса. "
            "Не объединяй пункты и не меняй порядок."
        )
    elif many_parts or long_query or multi_questions:
        q_for_model += (
            "\n\nЕсли в вопросе перечислено несколько пунктов или аспектов, "
            "обязательно ответь по каждому из них отдельным подпунктом, без пропусков."
        )

    # --- дальше твои гарды без изменений ---
    ...
    def _refusal(reason: str) -> str:
        base = (
            "В загруженном документе я не нашёл фрагментов, которые позволяют уверенно ответить на этот вопрос.\n"
            "Похоже, нужный текст не распарсился/не проиндексировался или сформулирован иначе.\n"
            "Попробуйте уточнить запрос (например, указать главу/раздел/страницу) или пришлите фрагмент текста."
        )
    ...
    if bool((facts or {}).get("no_grounding")):
        reason = (facts or {}).get("no_grounding_reason") or "no_grounding"
        return _refusal(str(reason))

    f = facts or {}
    general_ctx = (f.get("general_ctx") or "").strip()
    verbatim_hits = f.get("verbatim_hits") or []
    cov_items = ((f.get("coverage") or {}).get("items") or f.get("items") or [])
    tables_block = f.get("tables") or {}
    figures_block = f.get("figures") or {}

    has_tables = bool(tables_block) and (
        (tables_block.get("count") or 0) > 0
        or bool(tables_block.get("list"))
        or bool(tables_block.get("describe"))
    )
    has_figures = bool(figures_block) and (
        (figures_block.get("count") or 0) > 0
        or bool(figures_block.get("list"))
        or bool(figures_block.get("describe"))
        or bool(figures_block.get("describe_cards"))
    )
    has_sources = bool(f.get("sources") or {})
    has_summary = bool((f.get("summary_text") or "").strip())
    has_practical_flag = "practical_present" in f
    ...
    if not has_any_grounding:
        return _refusal("no_grounding_derived")
    ...
    if not (ctx or "").strip():
        return _refusal("empty_prompt_ctx")

    # --- NEW: усиливаем system_prompt требованием формата при явных пунктах ---
    system_prompt = (
        "Ты — строгий ассистент по ВКР.\n"
        "Тебе дан блок [Facts], извлечённый из документа.\n"
        "Отвечай ТОЛЬКО на основе [Facts]. Запрещено использовать общие знания, догадки или типовые определения.\n"
        "Если в [Facts] нет ответа на вопрос — так и скажи: «В документе не найдено».\n"
        "Не приписывай документу того, чего в нём нет.\n"
        "Если в вопросе перечислены пункты (1)/(2)/(3) или 1)/2)/3) — ты ОБЯЗАН ответить ровно тем же количеством пунктов, "
        "в том же порядке, и каждый пункт должен начинаться с '1)', '2)' и т.д. Без объединений.\n"
        "Если в вопросе упомянуты конкретные таблицы/рисунки/главы — сделай отдельные подпункты для каждого объекта в указанном порядке."
    )
    ...
    messages = [
        ...
        {"role": "user", "content": q_for_model},
    ]

    def _validate_answer_strict(ctx_text: str, answer_text: str) -> bool:
        if not chat_with_gpt:
            return True
        ctx_text = (ctx_text or "").strip()
        answer_text = (answer_text or "").strip()
        if not ctx_text or not answer_text:
            return False

        check_system = (
            "Ты строгий факт-чекер.\n"
            "Тебе даны [Facts] из документа и [Answer].\n"
            "Проверь: КАЖДОЕ утверждение в [Answer] должно быть явно подтверждено [Facts].\n"
            "Запрещено: допущения, обобщения, внешние знания.\n"
            "Верни строго один из вариантов:\n"
            "STRICT_OK\n"
            "или\n"
            "STRICT_FAIL"
        )
        check_user = f"[Facts]\n{ctx_text}\n\n[Answer]\n{answer_text}\n\nВердикт?"
        verdict = chat_with_gpt(
            [
        ...
        return "STRICT_OK" in verdict and "STRICT_FAIL" not in verdict

    # --- NEW: проверка покрытия пунктов и ремонт одним дополнительным запросом ---
    def _count_numbered_items(ans: str) -> int:
        # считаем строки, начинающиеся с "1)" "2)" ...
        return len(re.findall(r"(?m)^\s*\d{1,2}\)\s+", ans or ""))

    def _repair_missing_items(ctx_text: str, original_q: str, answer_text: str, expected_n: int) -> str:
        if not chat_with_gpt or expected_n <= 1:
            return answer_text

        have_n = _count_numbered_items(answer_text)
        if have_n >= expected_n:
            return answer_text

        repair_system = (
            "Ты — строгий ассистент по ВКР.\n"
            "Тебе дан блок [Facts] и черновик [Answer].\n"
            f"Нужно, чтобы итоговый ответ содержал РОВНО {expected_n} пунктов в формате 1), 2), ... {expected_n}).\n"
            "Если каких-то пунктов не хватает — ДОПИШИ их.\n"
            "Запрещено добавлять факты не из [Facts]. Если по пункту нет данных — напиши: "
            "«В документе не найдено». Ничего не объединяй и не меняй порядок."
        )
        repair_user = (
            f"[Facts]\n{ctx_text}\n\n"
            f"[Question]\n{original_q}\n\n"
            f"[Answer]\n{answer_text}\n\n"
            "Сформируй исправленный итоговый ответ."
        )
        fixed = chat_with_gpt(
            [
        ...
        return (fixed or "").strip() or answer_text
    ...
    try:
        ...
        final = _headings_to_bold((reply or "").strip())
        if not final:
            return _refusal("empty_model_reply")

        # NEW: если явно были пункты — ремонтируем пропуски (до факт-чека, чтобы чекать финал)
        if expected_n >= 2:
            final = _repair_missing_items(ctx, q, final, expected_n)
            final = _headings_to_bold((final or "").strip())

        if not final:
            return _refusal("empty_after_repair")

        if not _validate_answer_strict(ctx, final):
            return _refusal("strict_validation_failed")
...
async def generate_answer_stream(
    ...
    *,
    ...
    """
    Строгий стриминговый билдер:
      - всегда использует generate_answer(), потому что там:
          * строгие гарды
          * факт-чек
          * ремонт пропущенных пунктов
      - затем режет на чанки.
    """
    q = (question or "").strip()
    if not q:
        async def _empty():
