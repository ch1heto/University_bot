...
def _format_table_values_for_figure(table_num: str, ctx: str) -> str:
    """
    Преобразует сырой текст по таблице в более аккуратный вид:
    - выдёргивает табличные строки с разделителем " — ";
    - строит markdown-таблицу;
    - добавляет короткое связное пояснение.
    Если что-то пошло не так, возвращает исходный текст как есть.
    """
    ...
    for ln in lines:
        # выносим строку "Таблица 2.5 ..." отдельно, если есть
        if not title_line and ln.lower().startswith("таблица"):
            title_line = ln
        else:
            other_lines.append(ln)

    # собираем строки, похожие на строки таблицы, по разделителю " — "
    ...
    if not table_rows:
        # не смогли аккуратно собрать таблицу — просто аккуратно обернём исходный текст
        header = f"По данным таблицы {table_num} точные значения приведены ниже (как в документе):"
        if title_line:
            header += f"\n\n{title_line}"
        return header + "\n\n" + ctx
    ...
    if title_line:
        ...
        md.append("")
    md.append(f"По данным таблицы {table_num} точные значения (как в документе):")
    md.append("")
    # заголовок таблицы
    ...
    for row in data_rows:
        row = row[:len(header_cells)] + [""] * max(0, len(header_cells) - len(row))
        md.append("| " + " | ".join(row[:len(header_cells)]) + " |")
    ...
    *,
    ...
    for rec in records:
        ...
        if src == "ooxml":
            ...
            tbl = (rec.get("source_table_num") or "").strip()
            if tbl:
                title = f"**{disp} — значения по таблице {tbl} (как в документе)**"
            else:
