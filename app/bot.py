...
⚠️ **Важно:** Я работаю только с `.docx` файлами!
Если у тебя `.doc` — пересохрани:
 Открой документ → Файл → Сохранить как → Тип: *Word (.docx)*
...
async def handle_document_upload(m: types.Message):
    ...
    DOC_FALLBACK_MSG = (
        ...
        "К сожалению, я не могу обработать файлы в старом формате `.doc`. "
        "Но вы можете легко пересохранить файл в `.docx`!\n\n"
        "*Инструкция:*\n"
        "1️⃣ Откройте документ в Word\n"
        "2️⃣ Нажмите «Файл» → «Сохранить как»\n"
        "3️⃣ В поле «Тип файла» выберите *Документ Word (.docx)*\n"
        "4️⃣ Нажмите «Сохранить»\n\n"
        "После этого отправьте мне новый `.docx` файл! 📎"
    )
    ...
    try:
        ...
        # Логируем кратко, без полного traceback для типовых ошибок
        err_msg = str(e)
        if "CancelledError" in err_msg or "TimeoutError" in err_msg:
            logger.warning(f"Ошибка загрузки документа (user {uid}): {type(e).__name__}")
        else:
        ...
        is_doc_file = (
            ...
            not filename.lower().endswith('.docx')
        )
        ...
        if is_doc_file:
            ...
            await _send(m, DOC_FALLBACK_MSG, parse_mode="Markdown")
        else:
            await _send(m, "❌ Не удалось обработать документ. Попробуйте ещё раз.")