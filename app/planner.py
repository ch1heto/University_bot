...
def plan_tasks_from_user_query(
    ...
    На вход: сырой текст длинного запроса пользователя.
    На выход: упорядоченный список подзадач (Task), которые
    дальше можно по очереди обрабатывать в bot.py.

    max_tasks:
        Если задано — ограничиваем количество задач (например, 5-8),
        чтобы не распыляться в одном ответе.
    """
    clean = normalize_text(text)
    lower = clean.lower()
    tasks: List[Task] = []
    ...
    if theory_topics:
        tasks.append(
            Task(
                ...
                description=(
                    "Объяснить простым языком следующие темы: "
                    + ", ".join(theory_topics)
                ),
        ...
        tasks.append(
            Task(
                ...
                description=(
                    ...
                    "и организационную структуру на основе текста ВКР."
                ),
                original_text_span=text,
                priority=9,
            )
        ...
        tasks.append(
            Task(
                ...
                description=(
                    ...
                    "состояние предприятия: динамику, прибыль/убыток, "
                    "наличие проблем или устойчивость, на основе данных ВКР."
                ),
                original_text_span=text,
                priority=8,
            )
    ...
    # Не хотим путать номера разделов/рисунков с таблицами (например, "пункт 2.3")
    ...
    for ref in all_table_refs:
        tasks.append(
            Task(
                ...
                description=(
                    f"Проанализировать таблицу {ref}: структуру, динамику, "
                    f"темпы роста/сокращения, сделать выводы по финансовому положению."
                ),
                table_ref=ref,
                original_text_span=text,
                priority=7,
    ...
    for ref in figure_refs:
        tasks.append(
            Task(
                ...
                description=(
                    f"Найти рисунок(и) {ref} и проанализировать: что показано, "
                    f"как меняются показатели, какие выводы можно сделать."
                ),
                figure_ref=ref,
                original_text_span=text,
                priority=6,
    ...
    # Если упомянут "пункт 2.3" и при этом говорится про рисунки – создаём задачу по разделу
        ...
        for sec in section_refs:
            tasks.append(
                Task(
                    ...
                    description=(
                        f"Проанализировать рисунки/графики в пункте {sec}: "
                        f"описать динамику, тренды и сделать выводы."
                    ),
                    figure_ref=sec,
                    original_text_span=text,
                    priority=6,
    ...
    # 6. Проводки (если явно просили)
    if detect_postings_requested(clean):
        tasks.append(
            Task(
                type=TaskType.POSTINGS,
                title="Бухгалтерские проводки",
                description=(
                    "Пояснить типовые бухгалтерские проводки, а также "
                    "предложить примеры проводок по данным таблиц ВКР, "
                    "особенно связанных с запасами, обесценением и прочими операциями."
                ),
                original_text_span=text,
                priority=5,
            )
        ...
        tasks.append(
            Task(
                ...
                description=(
                    ...
                    "на основе предоставленного текста и данных."
                ),
                original_text_span=text,
                priority=0,
            )
    ...
    # Ограничиваем количество задач при необходимости
