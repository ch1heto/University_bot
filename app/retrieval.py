...
def retrieve(
    ...
    *,
    ...
    BASE_MIN_SCORE = float(getattr(Cfg, "RETRIEVE_MIN_SCORE", 0.24))

    def _is_facty_query(q: str) -> bool:
        ql = (q or "").lower()
        # универсальные маркеры перечислений/конкретики
        return bool(re.search(
            r"\b(какое|какая|какие|назовите|перечислите|использу|применя|"
            r"какая\s+программ|по\s+использ|инструмент|"
            r"методик|шкал|опросник|тест|модель|алгоритм|фреймворк|библиотек|стек|технолог)\b",
            ql
        ))

    MIN_SCORE = BASE_MIN_SCORE
    # Для фактологических вопросов допускаем чуть более слабые совпадения, чтобы не терять короткие факты
    if _is_facty_query(query):
        MIN_SCORE = max(0.12, BASE_MIN_SCORE - 0.06)
    ...
    # Но для коротких/фактологических запросов это часто ложнопусто.
    if not filtered and not special:
        # Берём 1–3 лучших чанка из rescored без порога,
        # чтобы не потерять потенциально правильные короткие факты.
        soft: List[Tuple[int, float]] = []
        sig2 = _query_signals(query)

        for i, sc in rescored[:3]:
            mi = pack["meta"][int(i)]

            # не тянем references, если не просили источники
            if (not sig2["ask_sources"]) and _chunk_type(mi) == "reference":
                continue

            # role-фильтр сохраняем
            if role and not _role_ok(mi):
                continue

            soft.append((int(i), float(sc)))
